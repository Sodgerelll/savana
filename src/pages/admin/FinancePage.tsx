...
import { ArrowDownCircle, ArrowUpCircle, ChevronLeft, ChevronRight, Pause, Pencil, Play, Plus, Repeat, Trash2, X } from "lucide-react";
...
export default function FinancePage({ ctx }: { ctx: AdminCtx }) {
  ...
  const monthEntries = useMemo(
    () => (financeEntries as FinanceEntryRecord[]).filter((e) => e.date.startsWith(monthPrefix)),
    [financeEntries, monthPrefix],
  );
    ...
    <>
      <div className="admin-topbar">
        <div>
          ...
          <p>
            {mn
              ? "Сар бүрийн орлого, зарлагыг өдрөөр бүртгэж, балансыг хянана."
              : "Record monthly income and expenses by day and monitor the balance."}
          </p>
        </div>
